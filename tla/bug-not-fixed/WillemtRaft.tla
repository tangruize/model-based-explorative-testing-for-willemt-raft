    ...
    LET xlog == log[s]
        F[i \in 0..Len(entries)] ==
            ...
            ELSE LET ety2     == GetEntryHelper(entries, i)
                     entries1 == IF IsNullEntryIdx(s, prevLogIdx + i)
                                 THEN xlog
                                 ELSE GetEntriesToIdx(s, prevLogIdx + i - 1)
...
SendSnapshotAllHelper(i) ==
    IF IsLeader(i)
    THEN LET except == { j \in Server: snapshotLastIdx'[i] <= nextIdx[i][j] }
         IN LockArgs(i, except, SnapshotRequest)
    ELSE ScrUnsetLock

SendSnapshot(i, j) ==
    /\ i /= j
    /\ IF snapshotLastIdx[i] > nextIdx[i][j]
...
AppendEntriesHelper(i, j) ==
    IF snapshotLastIdx'[i] > nextIdx'[i][j] THEN SendSnapshotHelper(i, j) ELSE
    ...
    IN IF b.term < currentTerm[i]    \* Term is smaller, reply false.
       THEN /\ UNCHANGED noNetworkVars
            ...
            /\ ScrSet(ScrIncSent @@ ScrIncRecvUnorder(m),
                        <<"HandleAppendEntriesRequest: smaller term",
                          i, j, m.seq, scr.nSent + 1, FALSE>>)
       ELSE /\ BecomeFollowerHelper(i, b.term, FALSE, TRUE, TRUE)
            /\ IF /\ b.prevLogIdx > 0  \* Check if prev log matches.
                  /\ NotEqualEntryTerm(i, b.prevLogIdx, b.prevLogTerm)
                  /\ b.prevLogIdx /= snapshotLastIdx[i]
               THEN \* Delete mismatch logs and reply false.
                    /\ UNCHANGED <<candidateVars, leaderVars, commitIdx,
                                   snapshotVars>>
                    /\ IF IsNullEntryIdx(i, b.prevLogIdx)
                       THEN UNCHANGED logStoreVars
                       ELSE LET tmp == GetEntriesToIdx(i, b.prevLogIdx - 1)
                                n == LogLen(i) - Len(tmp)
                    ...
                    /\ ScrSet(ScrIncSent @@ ScrIncRecvUnorder(m),
                                <<"HandleAppendEntriesRequest: log mismatch",
                                  i, j, m.seq, scr.nSent + 1, FALSE>>)
               ELSE \* Prev log matches, append entries and reply true.
...
HandleSnapshotRequest(i, j, m) ==
    ...
    IN IF \/ b.term < currentTerm[i]
          ...
          \/ b.lastTerm = 0
          \/ b.lastIdx < LastIdx(i)
          \/ b.lastTerm = snapshotLastTerm[i] /\ b.lastIdx = snapshotLastIdx[i]
       THEN /\ UNCHANGED noNetworkVars
       ...
       ELSE /\ UNCHANGED <<candidateVars, leaderVars>>
            ...
            /\ currentTerm' = [ currentTerm  EXCEPT ![i] = b.lastTerm ]
...
\* Last modified Sat Aug 21 22:52:19 CST 2021 by tangruize
